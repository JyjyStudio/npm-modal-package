...
3.  Create a `<Modal>` component with isShowing and hide props and nest its content inside of it:
...
export function AppTester() {
	...
	return (
		<>
			...
			<Modal isShowing={isShowing} hide={toggle}>Modal successfully created !</Modal>
		</>
	)
}
```
